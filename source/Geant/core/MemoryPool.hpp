...
class MemoryPage
{
    ...
    static_assert(std::is_same<_Mem, memory::host>::value || std::is_same<_Mem, memory::device>::value ||
...
public:
    ...
    template <typename _Up = _Mem, std::enable_if_t<(!std::is_same<_Up, memory::device>::value), int> = 0>
...
private:
    ...
    //
    template <typename _Up = _Mem, std::enable_if_t<(std::is_same<_Up, memory::host>::value), int> = 0>
    static type* _alloc()
    {
        return static_cast<type*>(std::malloc(GetPageSize()));
    }

    template <typename _Up = _Mem, std::enable_if_t<(std::is_same<_Up, memory::pinned>::value), int> = 0>
    static type* _alloc()
    {
        void* _page = nullptr;
...
public:
    ...
    {
        return (m_associated.find(_cpu) != m_associated.end()) ? m_associated.find(_cpu)->second : nullptr;
    }

    template <typename _Target, std::enable_if_t<(std::is_same<_Target, device::gpu>::value), int> = 0>
    ...
    template <typename _Target, std::enable_if_t<(std::is_same<_Target, device::cpu>::value), int> = 0>
    void transfer_to(cudaStream_t stream = 0)
    {
        for(auto& itr : m_associated)
