...
double Proxy2DVector::Value(double x, double y)
{
  ...
  if(x < xVector[0]) {
    x = xVector[0];
  } else if(x > xVector[data::numberOfXNodes - 1]) {
    x = xVector[data::numberOfXNodes - 1];
  }
  if(y < yVector[0]) {
    y = yVector[0];
  } else if(y > yVector[data::numberOfYNodes - 1]) {
    y = yVector[data::numberOfYNodes - 1];
  }
...
size_t Proxy2DVector::FindBinLocationX(double z)
{
  ...
  else if(z >= xVector[data::numberOfXNodes-2]) {
    id = data::numberOfXNodes - 2;
  }
  else {
    size_t lowerBound = 0;
    size_t upperBound = data::numberOfXNodes - 2;
...
size_t Proxy2DVector::FindBinLocationY(double z)
{
  ...
  else if(z >= yVector[data::numberOfYNodes-2]) {
    id = data::numberOfYNodes - 2;
  }
  else {
    size_t lowerBound = 0;
    size_t upperBound = data::numberOfYNodes - 2;
