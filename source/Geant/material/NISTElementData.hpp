...
public:
   ...
   */
  std::string GetElementSymbol(const int z) const
  {
    return fNISTElementDataTable[z - 1].fElemSymbol;
  }

  /** @brief Public method to access number of known isotopes of a given element.
   *
   * @param[in] z Atomic number of the requested element.
   * @return    Number of known isotopes of the element specified by the given atomic
   * number.
   */
  int GetNumberOfIsotopes(const int z) const
  {
    return fNISTElementDataTable[z - 1].fNumOfIsotopes;
  }

  /** @brief Public method to get the index of the most stable isotope of this element.
   *
   * If the element has no stable isotope we take the most stable isotope: when user build
   * an element without specifying the isotopes of the element, this database is used and
   * isotopes of the given element with non zero natural abundances are insterted into the
   * element. However, there are elements that has no stable isotope. In this case the
   * this procedure would lead to an element without having any isotopes. IN order to
   * avoid this situtation, we will take the most stable isotope of the element and will
   * inform the user.
   *
   * @param[in] z Atomic number of the requested element.
   * @return    Index of the most stable isotope if the elemnet has no stable isotope.
   * Will return with -1 if the element has stable isotope.
   */
  ...
  /** @brief Public method to get the flag that indicates if this element has no stable
   * isotope.
   *
   * See more at GetIndexOfTheMostStableIsotope().
   *
   * @param[in] z Atomic number of the requested element.
   * @return    True if the element has no stable isotope. The index of the most stable
   * isotope can be got through GetIndexOfTheMostStableIsotope(). False if the element has
   * stable isotope.
   */
  bool IsHaveNoStableIsotope(const int z) const
  {
    return fNISTElementDataTable[z - 1].fIsNoStableIsotope;
  }

  /** @brief Public method to access symbols of all known isotopes of a given element.
   *
   * @param[in] z Atomic number of the requested element.
   * @return    Symbols of all known isotopes of the element specified by the given atomic
   * number. Size of the array is equal to GetNumberOfIsotopes().
   */
  const std::string *GetIsotopeSymbols(const int z) const
  {
    return fNISTElementDataTable[z - 1].fSymbols;
  }

  /** @brief Public method to access nucleon numbers of all known isotopes of a given
   * element.
   *
   * @param[in] z Atomic number of the requested element.
   * @return    Nucleon numbers of all known isotopes of the element specified by the
   * given atomic number. Size of the array is equal to GetNumberOfIsotopes().
   */
  const int *GetIsotopeNucleonNums(const int z) const
  {
    return fNISTElementDataTable[z - 1].fNIsos;
  }

  /** @brief Public method to access atomic masses of all known isotopes of a given
   * element.
   *
   * Atomic masses include the mass of the electron shell as well. Atomic masses are
   * stored in internal [weigth] units and can be converted to internal [weight/amount of
   * substance] units multiplying by the Avogadro(kAvogadro) number OR to internal
   * [energy] unit multiplying by speed of light square (kCLightSquare).
   *
   * @param[in] z Atomic number of the requested element.
   * @return    Atomic masses of all known isotopes of the element specified by the given
   * atomic number. Size of the array is equal to GetNumberOfIsotopes().
   */
  const double *GetIsotopeAtomicMasses(const int z) const
  {
    return fNISTElementDataTable[z - 1].fAIsos;
  }

  /** @brief Public method to access isotope masses of all known isotopes of a given
   * element.
   *
   * Isotope masses are the masses of the bare nucleus in internal [energy] units i.e.
   * without the electron shell and the corresponding total electron binding energy.
   *
   * @param[in] z Atomic number of the requested element.
   * @return    Masses of all known isotopes of the element specified by the given atomic
   * number. Size of the array is equal to GetNumberOfIsotopes().
   */
  const double *GetIsotopeMasses(const int z) const
  {
    return fNISTElementDataTable[z - 1].fMassIsos;
  }

  /** @brief Public method to access natural abundances of all known isotopes of a given
   * element.
   *
   * @param[in] z Atomic number of the requested element.
   * @return    Natural abundances of all known isotopes of the element specified by the
   * given atomic number. Size of the array is equal to GetNumberOfIsotopes().
   */
  const double *GetIsotopeNaturalAbundances(const int z) const
  {
    return fNISTElementDataTable[z - 1].fWIsos;
  }
   ...
   */
  double GetMeanAtomicMass(const int z) const
  {
    return fNISTElementDataTable[z - 1].fMeanAtomicMass;
  }
   ...
   */
  //@{
  /** @brief Public method to access the already built NIST element indices in the global
   * element table.
   *
   ...
   * @return
   *            - Index of the specified NIST element if that NIST element has already
   * built.
   *            - -1 otherwise.
   */
...
private:
  ...
  struct NISTElement {
    ...
    /** @brief Number of nucleons for each known isotopes of this element
     * [fNumOfIsotopes]. */
    const int *fNIsos;
    /** @brief Atomic masses in internal [weight] units for each known isotopes of this
     * element [fNumOfIsotopes]. */
    const double *fAIsos;
    /** @brief Natural abundances for each known isotopes of this element
     * [fNumOfIsotopes]. */
    const double *fWIsos;
    /** @brief Masses for each known isotopes of this element in internal [energy] units
     * [fNumOfIsotopes]. */
    double *fMassIsos;
