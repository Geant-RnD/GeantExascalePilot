 ...
 *
 * The density effect parameter \f$ \delta(X) \f$ where \f$ X = \log_{10}(pc/(m_0 c^2)) =
 *\log_{10}(\gamma\beta) \f$ (with \f$ pc \f$ total momentum in energy units, \f$ m_0 c^2
 *\f$ rest mass energy, \f$ \beta=v/c \f$ and \f$ \gamma = (1-\beta^2)^{-1/2} \f$):
 * -# is computed according to the parametrisation given in \cite sternheimer1984density
 *(Eqs.(14,9,10)) if the material can be identified in the internal DensityEffectData
 *database based on its name (see \ref DensityEffectDataDoc). Namely \f[ \delta(X) =
 *       \begin{cases}
 ...
 *       \end{cases}
 *    \f]
 *    where \f$ \delta(X_0), X_0, X_1, a, m \f$ parameters are taken from the
 *DensityEffectData database, \f$\tilde{C} = -C = 2\ln(I/(h\nu_p)) + 1\f$ where \f$ I \f$
 *is the mean excitation energy (see above) and \f$ h\nu_{p} \f$ is the plasma energy of
 *the electrons of the material considered as free electron gas. Parameters \f$ \tilde{C}
 *= -C, h\nu_{p} \f$ are also taken from the DensityEffectData database if the material
 *can be identified based on its name. Note, that the density effect parameters were
 *determined in \cite sternheimer1984density through fitting the above formula to computed
 *\f$ \delta(X)\f$ values for the given materials.
 * -# if the material cannot be identified in the DensityEffectData database based on its
 *name:
 ...
 *      \f]
 *      where \f$ \mathcal{N}_{av} \f$ is the Avogadro number, \f$ \rho \f$ is the
 *material density, \f$ w_i \f$ is the fractional mass of the \f$ i \f$-th element of the
 *material with molar mass of \f$ A_i \f$.
 *    2. then the general formula given in \cite sternheimer1971general is used to compute
 *the density effect parameters. Namely \f[ \delta(X) = \begin{cases}
 *          \delta(X_0) = 0                        & \quad  X \leq X_0   \\
 *          2\ln(10)X + a(X_1-X)^m - \tilde{C}     & \quad  X_0 < X <X_1 \\
 *          2\ln(10)X - \tilde{C}                  & \quad  X \geq X_1
 *         \end{cases}
 *      \f]
 *      where \f$\tilde{C} = -C = 2\ln(I/(h\nu_p)) + 1\f$ as mentioned above, \f$ m = 3\f$
 *is fixed. The value of \f$ a \f$ is determined by the requirement \f$ \delta(X=X_0) = 0
 *= 2\ln(10)X_0 + a(X_1-X_0)^m - \tilde{C} \f$ and gives \f$ a =
 *[\tilde{C}-2\ln(10)X_0]/[X_1-X_0]^m\f$ that can be written as \f$ a =
 *2\ln(10)[X_a-X_0]/[X_1-X_0]^m\f$ with \f$ X_a=\tilde{C}/(2\ln(10))\f$. The values of the
 *remaining parameters \f$ X_0, X_1 \f$ according to \cite sternheimer1971general
 *      - for solids and liquids(Eqs.(33-36) in \cite sternheimer1971general)
 *      \f[
 *         I < 100 \textrm{[eV]}
 *         \begin{cases}
 *          X_0 = 0.2,                 & X_1 = 2.0   & \textrm{for} \quad  \tilde{C}  <
 *3.681  \\
 *          X_0 = 0.326\tilde{C}-1.0,  & X_1 = 2.0   & \textrm{for} \quad  \tilde{C}
 *\geq 3.681  \\ \end{cases} \f] \f[ I \geq 100 \textrm{[eV]} \begin{cases}
 *          X_0 = 0.2,                 & X_1 = 3.0   & \textrm{for} \quad  \tilde{C}  <
 *5.215  \\
 *          X_0 = 0.326\tilde{C}-1.5,  & X_1 = 3.0   & \textrm{for} \quad  \tilde{C}
 *\geq 5.215  \\ \end{cases} \f]
 *      - for gases(Eqs.(37-41) plus text in \cite sternheimer1971general)
 *      \f[
 *         \begin{matrix}
 *          X_0 &=& 1.6,                 & X_1 = 4.0   & \textrm{for} &        &      &
 *\tilde{C} & < & 10.0  \\
 *          X_0 &=& 1.7,                 & X_1 = 4.0   & \textrm{for} & 10.0   & \leq &
 *\tilde{C} & < & 10.5  \\
 *          X_0 &=& 1.8,                 & X_1 = 4.0   & \textrm{for} & 10.5   & \leq &
 *\tilde{C} & < & 11.0  \\
 *          X_0 &=& 1.9,                 & X_1 = 4.0   & \textrm{for} & 11.0   & \leq &
 *\tilde{C} & < & 11.5  \\
 *          X_0 &=& 2.0,                 & X_1 = 4.0   & \textrm{for} & 11.5   & \leq &
 *\tilde{C} & < & 12.25  \\
 *          X_0 &=& 2.0,                 & X_1 = 5.0   & \textrm{for} & 12.25  & \leq &
 *\tilde{C} & < & 13.804  \\ X_0 &=& 0.326\tilde{C}-2.5,  & X_1 = 5.0   & \textrm{for}
 *& 13.804 & \leq & \tilde{C} &   & \end{matrix} \f]
 * -# the last point is to account the fact that the density effect parameters for gaseous
 *materials were derived under the conditions \f$ T =
 *0[^{\circ}\textrm{C}]\;\textrm{and}\; P = 1 \textrm{[atm]}\f$ and the corresponding gas
 *    density is \f$ \rho_0 \f$. According to Eq.(44) in \cite sternheimer1971general, if
 *the gas density is \f$ \rho=\eta\rho_0 \f$, we need to evaluate \f$ \delta \f$ at \f$
 *p\eta^{1/2}\f$ momentum instead of \f$ p \f$. Since \f$ X=\log_{10}(p/(m_0 c)) =
 *\log_{10}(p)-\log_{10}(m_0c)\f$ \f$ \to X_{\eta} \equiv
 *\log_{10}(p)+0.5\log_{10}(\eta)-\log_{10}(m_0c) = X+0.5\log_{10}(\eta)\f$, elavulating
 *    \f$ \delta \f$ at \f$ p\eta^{1/2}\f$ momentum corresponds to \f$
 *\delta(X_{\eta}\equiv X+0.5\log_{10}(\eta)) \f$. Therefore, evaluating \f$ \delta \f$ at
 *momentum \f$ p\eta^{1/2}\f$ instead of \f$ p \f$ corresponds to taking the value of \f$
 *\delta \f$ at a shifted \f$ X \f$. Since \f$ X \f$ is shifted to \f$
 *X+0.5\log_{10}(\eta) \f$ it corresponds to the shift of the two limits in \f$ X \f$ i.e.
 *      \f[
 *         \begin{matrix}
 ...
 *      \f]
 *    It can be seen from the condition for \f$ a \f$ that parameters \f$ a,\; m \f$
 *remains the same i.e \f[ a = \frac{\tilde{C}-2\ln(10)X_0}{[X_1-X_0]^m} =
 *\frac{\tilde{C}_{\eta}-2\ln(10)X_{0,\eta}}{[X_{1,\eta}-X_{0,\eta}]^m} \f]
 *    and \f$ X_a \f$ now becomes \f$ X_{a,\eta} \equiv \tilde{C}_{\eta}/(2\ln(10)) \f$.\\
 *    It is important to note, that this correction can also be applied to solids and
 *liquids that has a density different from that was used to derive the density effect
 *parameters (ro data).
 *
 *\todo
 * -# Add possibility to set mean excitation energy.
 * -# Add possibility to set density effect parameters directly (X0,X1,a,m,Cbar).
 * -# The best would be to use numerical \f$ \delta(X) \f$ values that were used to derive
 *the fit parameters. Such data avaliable for a significant set of simple and composite
 *materials (EGS simulations use this) If we take those data, the data file will contain
 *the mean ionisation energy (I[eV]), the density (\f$ \rho_0 \f$) that were used in the
 *computation and the \f$ E_{kin},\; \delta(E_{kin}) \f$ datapairs for electrons. What we
 *need to do is:
 *    - first we need to change the \f$ E_{kin} \f$ scale to \f$
 *X=log_{10}(pc/(m_0c^2))\f$. Since \f$ (pc/(m_0c^2)) = \sqrt{
 *E_{kin}/(m_0c^2)[E_{kin}/(m_0c^2)+2]} \f$ where \f$ m_0 c^2 \f$ is the electron rest
 *mass energy,
 *      \f$ X = log_{10} (\sqrt{ E_{kin}/(m_0c^2)[E_{kin}/(m_0c^2)+2]}) \f$.
 *    - then we need to apply possible correction comming from the possibility that the
 *density hat was assumed during the computation of these \f$ \delta(E_{kin}) \f$ data
 *(\f$ \rho_0 \f$) and our current density (\f$ \rho \f$) are different. We can do this by
 *simple shifting the \f$ X \f$ scale computed above to \f$ X = X - 0.5\log_{10}(\eta) \f$
 *      where \f$ \eta = \rho/\rho_0 \f$.
 *    - it's also interesting to note, that in the upper limiting part of \f$ X \f$, \f$
 *\delta(X) = 2\ln(10)X - \tilde{C}\f$ where \f$\tilde{C} = 2\ln(I/(h\nu_p)) + 1\f$.
 *Plugging this later into the previous gives \f$ I/(h\nu_p) =
 *\exp{[X\ln(10)-0.5(\delta(X)+1)]} \f$ i.e. taking the \f$ X,\; \delta(X) \f$  pair that
 *corresponds
 *      to the highest value of \f$ X \f$ gives the possiblity of getting \f$ h\nu_p \f$
 *if the \f$ I \f$ was given in the file.
 *
 */
class Material;
...
public:
  ...
  /** @brief Public method to get the total number of aelectrons per volume in this
   * material.
   ...
   *  @return Effective atomic number: number of electons per volume per number of atoms
   * per volume.
   */
  double GetEffectiveZ() const { return fZeff; }

  /** @brief Public method to get the number of atoms per volume for each elements this
   ...
   * param[in]  val This is \f$ X\equiv \log_{10} (\gamma\beta) =
   * \ln(\gamma^2\beta^2)/(2\ln(10))\f$. return     Density effect correction parameter
   * \f$ \delta(X)\f$.
   */
...
private:
  /** @brief Pointer to the material that this material properties belong to (doesn't own
   * the object)*/
  Material *fMaterial;
