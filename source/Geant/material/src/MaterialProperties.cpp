...
void MaterialProperties::ComputeBasicMaterialParameters()
{
  ...
  for (int i = 0; i < numElems; ++i) {
    ...
    double aeff =
        elemVector[i]->GetA(); // atomic mass in internal [weight/amount of substance]
...
void MaterialProperties::ComputeIonizationParameters()
{
  ...
  if (indx >
      -1) { // material could be identified by its name in the NISTMaterialData database
  ...
  if (fMeanExcitationEnergy <= 0.0) {
    fMeanExcitationEnergy = 0.0;
    // get the max Z(atomic number) that we have elemental data for in the
    // NISTMaterialData database
...
void MaterialProperties::ComputeDensityEffectParameters()
{
  ...
  using geantx::units::kClassicElectronRadius;
  using geantx::units::kHBarPlanckCLightSquare;
  using geantx::units::kPi;
  using geantx::units::kSTPPressure;
  using geantx::units::kSTPTemperature;
  ...
  // get the material name and check if we take density effect parameters from the
  // DensityEffectData we take the parameters only if the material is a NIST material and
  // we can find data in the DB
  int indx =
      DensityEffectData::Instance().FindDensityEffectDataIndex(fMaterial->GetName());
  ...
  // - Imean should be the same as in the NIST_MAT_! (by default it is ok but can be set
  // to be different!)
  ...
  if (indx < 0 && numElems == 1) {
    ...
    indx   = DensityEffectData::Instance().GetElementalIndex(z0,
                                                           fMaterial->GetMaterialState());
    ...
    if (indx > 0) { // simple NIST material DenistyEffectData was found with the correct
                    // state we have the corresponding NISTMaterialData as well so get the
                    // denisty from there
  ...
  // check if material was found in the DensityEffectData database (IMPORTANT:: Mean
  // excitation energy should be the NIST one i.e. material should be the NIST one !)
  if (indx >= 0) {
    fPlasmaEnergy = DensityEffectData::Instance().GetPlasmaEnergy(indx);
    // we always recompute the Cbar parameter
    // fDensityEffectParameterC       = 2.0*Math::Log(fMeanExcitationEnergy/fPlasmaEnergy)
    // + 1;
    ...
    fDensityEffectParameterDelta0 =
        DensityEffectData::Instance().GetParameterDelta0(indx);

    // apply density correction: note, that if it was a real nist material then corrFactor
    // = 0 at this point and if it was not a nist material but single element material
    // (case 2. that indx>0)
    fDensityEffectParameterC -= corrFactor; // cbar_\eta = cbar - \ln(10)log10(\eta)
    fDensityEffectParameterX0 -=
        corrFactor / twolog10; // X_{1\eta} = X_1 - 0.5log10(\eta)
    fDensityEffectParameterX1 -=
        corrFactor / twolog10; // X_{0\eta} = X_0 - 0.5log10(\eta)
  } else {
    // we compute the density effect parameters based on:
    // Sternheimer, R. M., and R. F. Peierls. "General expression for the density
    // effect for the ionization loss of charged particles." Physical Review B 3.11
    // (1971): 3681.
    ...
    fPlasmaEnergy =
        std::sqrt(dum0 * fTotalNumOfElectronsPerVol); // in internal [energy] units
    // paramater \f$ -C = 2\ln{I/(h\nu_p)} +1 \f$
    fDensityEffectParameterC =
        2.0 * Math::Log(fMeanExcitationEnergy / fPlasmaEnergy) + 1.0;
    fDensityEffectParameterM = 3.0;
    // other parameters depends on the state of the material
    // for solids and liquids
    if (matState == MaterialState::kStateSolid ||
        matState == MaterialState::kStateLiquid) {
      if (fMeanExcitationEnergy < 100.0 * eV) { // Imean<100 eV
        fDensityEffectParameterX1 = 2.0;
        fDensityEffectParameterX0 = 0.2;
        if (fDensityEffectParameterC >= 3.681)
          fDensityEffectParameterX0 = 0.326 * fDensityEffectParameterC - 1.0;
      } else { // Imean>=100 eV
        fDensityEffectParameterX1 = 3.0;
        fDensityEffectParameterX0 = 0.2;
        if (fDensityEffectParameterC >= 5.215)
          fDensityEffectParameterX0 = 0.326 * fDensityEffectParameterC - 1.5;
      }
      // Liquid Hydrogen is special [Table 9 in Seltzer Berger : Evaluation of the
      // collision stopping power of elements and compounds for electrons and
      // positrons(1981)]
      int z0 = std::lrint(fMaterial->GetElementVector()[0]->GetZ());
  ...
  if (matState == MaterialState::kStateGas) {
    ...
    // double stpDenisty     =
    // curDensity*curTemperature*kSTPPressure/(curPressure*kSTPTemperature);
    corrFactor =
        Math::Log(curPressure * kSTPTemperature / (curTemperature * kSTPPressure));
  ...
  // if \f$\delta(X=X_0)=0\f$ then parameter 'a' must staisfy the \f$ 0 = 2\ln(10)X +
  // a(X_1-X_0)^m + C\f$ at X=X_0
  if (fDensityEffectParameterDelta0 == 0.0) {
    double dum0 = Math::Pow(fDensityEffectParameterX1 - fDensityEffectParameterX0,
                            fDensityEffectParameterM);
    fDensityEffectParameterA =
        (fDensityEffectParameterC - twolog10 * fDensityEffectParameterX0) / dum0;
  }
}
...
double MaterialProperties::GetDensityEffectFunctionValue(const double atx)
{
  ...
  if (atx < fDensityEffectParameterX0) { // below lower limit i.e. either zero or for
                                         // conductors might be a small constant
    // conductor and delta converge to non-zero at low momentum/mass values
    if (fDensityEffectParameterDelta0 >
        0.0) // delta(X) = delta(X_0) 10^{2(X-X_0)} = delta(X_0) exp[2ln(10)(X-X_0)]
      delta = fDensityEffectParameterDelta0 *
              Math::Exp(twolog10 * (atx - fDensityEffectParameterX0));
  } else if (atx >= fDensityEffectParameterX1) {
    delta = twolog10 * atx - fDensityEffectParameterC;
  } else {
    delta =
        twolog10 * atx +
        fDensityEffectParameterA * Math::Exp(fDensityEffectParameterM *
                                             Math::Log(fDensityEffectParameterX1 - atx)) -
        fDensityEffectParameterC;
  }
  return delta;
}
...
void MaterialProperties::ComputeRadiationLength()
{
  ...
  constexpr double factor = 4.0 * geantx::units::kFineStructConst *
  ...
  for (int i = 0; i < numElems; ++i) {
    ...
    double mu  = zet * geantx::units::kFineStructConst;
    double mu2 = mu * mu;
    double mu4 = mu2 * mu2;
    double mu6 = mu2 * mu4;
    double coulombCorrection =
        mu2 * (1.0 / (1.0 + mu2) + 0.20206 - 0.0369 * mu2 + 0.0083 * mu4 - 0.002 * mu6);
    // compute radlength
    double logZper3 = Math::Log(zet) / 3.0;
    double dum0     = 0.0;
    if (zet <= 3) { // it should be < 5 in order to be consistent with the relativistic
                    // brems. models
      dum0 = zet * zet * (FelLowZet[izet] - coulombCorrection) +
             zet * FinelLowZet[izet]; //+1/18.*zet*(zet+1.0);
    } else {
  ...
  fRadiationLength =
      (invRadLength <= 0.0 ? std::numeric_limits<double>::max() : 1.0 / invRadLength);
}
