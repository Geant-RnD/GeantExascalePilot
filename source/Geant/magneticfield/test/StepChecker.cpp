...
bool StepChecker::CompareStep(
    vecgeom::Vector3D<double> const &Position, vecgeom::Vector3D<double> const &Direction,
    ...
    ) const
{
  ...
  if (fVerbose) {
    printf("Particle with charge %g  momentum = %8.4g Position= %10.6f %10.6f %10.6f "
           ...
           charge, momentum, Position[0], Position[1], Position[2], Direction[0],
    ...
    printf(" Step=%8f  Move/RK4:  Pos/shift= %10.6f  %10.6f  %10.6f  Mom/shft= "
           "%10.3g %10.3g %10.3g \n",
           step, PositionShift[0], PositionShift[1], PositionShift[2], DirectionShift[0],
           DirectionShift[1], DirectionShift[2]);
  }
  ...
  if (differ) {
    printf(" Step=%8f    DIFFERS   Pos/diff= %10.6f  %10.6f  %10.6f  Mom/diff= "
           "%10.3g %10.3g %10.3g \n",
           step, PositionDiff[0], PositionDiff[1], PositionDiff[2], DirectionDiff[0],
           DirectionDiff[1], DirectionDiff[2]);
    ...
    printf(" Step=%8f  Helix/Mv:  Pos/shift= %10.6f  %10.6f  %10.6f  Mom/shft= "
           "%10.3g %10.3g %10.3g \n",
           step, PositionShiftH[0], PositionShiftH[1], PositionShiftH[2],
...
bool StepChecker::CheckStep(vecgeom::Vector3D<double> const &Position,
                            vecgeom::Vector3D<double> const &Direction, double charge,
                            double momentum, double step,
                            vecgeom::Vector3D<double> const &endPosition,
                            vecgeom::Vector3D<double> const &endDirection,
                            vecgeom::Vector3D<double> const &BfieldVec) const
{
  ...
  stepper.DoStep<double>(Position, Direction, charge, momentum, step, PositionNewHelix,
                         DirectionNewHelix);
#else
  // More complicated version ...
  if (std::fabs(BfieldVec[2]) >
      1e6 * std::max(std::fabs(BfieldVec[0]), std::fabs(BfieldVec[1]))) {
  ...
  return CompareStep(Position, Direction, charge, momentum, step, endPosition,
                     endDirection, PositionNewHelix, DirectionNewHelix);
}
