...
public:
  ...
  inline // GEANT_FORCE_INLINE
      void
      RightHandSide(const double y[], const Vector3D<double> &position, double charge,
                    double dydx[], Vector3D<double> &BfieldVec) const;
  ...
  virtual void EvaluateRhsGivenB(const double y[], const Vector3D<double> &B,
                                 double charge, double dydx[]) const;

  template <typename Real_v>
  void EvaluateRhsGivenB(const Real_v y[], const Vector3D<Real_v> &B,
  ...
  GEANT_FORCE_INLINE
  void TEvaluateRhsReturnB(const double y[], double dydx[], double charge,
  ...
  GEANT_FORCE_INLINE
  void PrintAll(double const y[], const vecgeom::Vector3D<double> &B, double charge,
...
// fPtrField( new Field(right.fPtrField) )
{
  ...
  VScalarEquationOfMotion::SetFieldObj(
    ...
    bool &safe)
{
  ...
  std::cerr
      << " #ScalarMagFieldEquation<Field,Size>::CloneOrSafeSelf(bool& safe) called# "
      << std::endl;
...
{
  bool safeLocal;
  std::cerr << " #ScalarMagFieldEquation<Field,Size>::CloneOrSafeSelf(bool* safe) called#"
            << std::endl;
    ...
    double const dydx[]) const
{
  ...
  printf("Equation:  gCof = %8.4g  charge= %f cof= %10.5g   B-field= %f %f %f \n", gCof,
         charge, cof, B[0], B[1], B[2]);
  // printf("               X  = %12.6g %12.6g %12.6g - mag %12.6g\n",  y[0], y[1], y[2]
  // );

  printf("            dx/ds  = %12.6g %12.6g %12.6g - mag %12.6g\n", dydx[0], dydx[1],
         dydx[2], std::sqrt(dydx[0] * dydx[0] + dydx[1] * dydx[1] + dydx[2] * dydx[2]));
  printf("            dp/ds  = %12.6g %12.6g %12.6g - mag %12.6g\n", dydx[3], dydx[4],
         dydx[5], std::sqrt(dydx[3] * dydx[3] + dydx[4] * dydx[4] + dydx[5] * dydx[5]));

  double Bmag2chk = B[0] * B[0] + B[1] * B[1] + B[2] * B[2];
  printf("            B-field= %10.3f %10.3f %10.3f  ( KGaus ) mag= %10.4f\n",
         B[0] / geantx::units::kilogauss, B[1] / geantx::units::kilogauss,
         B[2] / geantx::units::kilogauss, std::sqrt(Bmag2chk));
