...
# - Include guard
if(NOT __CLANGFORMAT_INCLUDED)
    set(__CLANGFORMAT_INCLUDED 1)
else()
    return()
endif()
...
set(FORMAT_LOGDIR ${PROJECT_BINARY_DIR}/format CACHE PATH
...
function(create_format_directory)
    if(NOT EXISTS ${FORMAT_LOGDIR})
        execute_process(COMMAND ${CMAKE_COMMAND}
            -E make_directory ${FORMAT_LOGDIR})
    endif()
endfunction()

#------------------------------------------------------------------------------#

FUNCTION(geant_format_target)
    if(CLANG_FORMAT_COMMAND)
        cmake_parse_arguments(FMTTARGET
            "" "NAME" "SOURCES" ${ARGN})
        string(REGEX REPLACE "-format$" "" TARGET_NAME "${FMTTARGET_NAME}")
        # ensure not empty list
        if(NOT "${FMTTARGET_SOURCES}" STREQUAL "")
            # create output directory
            create_format_directory()
            set(SOURCES ) # list of sources
            set(FILELOG ) # string of sources (for log)
            foreach(_SOURCE ${FMTTARGET_SOURCES})
            ...
            endforeach()
            # write format file
            set(FMTLOG_OUT ${FORMAT_LOGDIR}/${TARGET_NAME}.txt)
            ...
            if(NUM_SOURCES GREATER 500)
                ...
                while(NUM_SOURCES GREATER 0)
                    ...
                    list(APPEND FORMAT_COMMANDS COMMAND ${CLANG_FORMAT_COMMAND} -i ${SOURCES_SUBSET})
                ...
                if(NUM_COMMANDS GREATER 0)
                    add_custom_target(${FMTTARGET_NAME}
                        ${FORMAT_COMMANDS}
                        WORKING_DIRECTORY ${PROJECT_SOURCE_DIR}
                        COMMENT
                        "Running '${CLANG_FORMAT_COMMAND}' on source files in '${TARGET_NAME}' target (see ${FMTLOG_RELATIVE_OUT})...")
                endif()
            else()
                list(LENGTH SOURCES NUM_SOURCES)
                if(NUM_SOURCES GREATER 0)
                    ...
                    add_custom_target(${FMTTARGET_NAME}
                        COMMAND ${CLANG_FORMAT_COMMAND} -i ${SOURCES}
                        WORKING_DIRECTORY ${PROJECT_SOURCE_DIR}
                        COMMENT
                        "Running '${CLANG_FORMAT_COMMAND}' on source files in '${TARGET_NAME}' target (see ${FMTLOG_RELATIVE_OUT})...")
                endif()
