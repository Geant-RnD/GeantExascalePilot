...
DetectorConstruction::DetectorConstruction()
    : G4VUserDetectorConstruction(), fAbsorberMaterial(0), fWorldMaterial(0),
      fDefaultWorld(true), fSolidWorld(0), fLogicWorld(0), fPhysiWorld(0),
      fSolidAbsorber(0), fLogicAbsorber(0), fPhysiAbsorber(0), fPrimaryGenerator(0),
...
void DetectorConstruction::DefineMaterials()
{
  ...
  G4Material *rock1 =
      new G4Material("StandardRock", 2.65 * CLHEP::g / CLHEP::cm3, 1, kStateSolid);
  ...
  new G4Material("Galactic", z = 1, a = 1.01 * g / mole, density, kStateGas, temperature,
                 pressure);
}
...
G4VPhysicalVolume *DetectorConstruction::ConstructCalorimeter()
{
  ...
  // Absorber
  //
  fSolidAbsorber = new G4Box("Absorber", fAbsorberThickness / 2, fAbsorberSizeYZ / 2,
                             fAbsorberSizeYZ / 2);
  ...
  //  G4GDMLParser parser;
  //  parser.Write("hanson_9.gdml", fPhysiWorld);
...
void DetectorConstruction::PrintCalorParameters()
{
  ...
  G4cout << " The ABSORBER is made of " << G4BestUnit(fAbsorberThickness, "Length")
         << " of " << fAbsorberMaterial->GetName();
  G4cout << ". The transverse size (YZ) is " << G4BestUnit(fAbsorberSizeYZ, "Length")
         << G4endl;
