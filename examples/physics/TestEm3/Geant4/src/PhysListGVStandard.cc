...
PhysListGVStandard::PhysListGVStandard(const G4String &name) : G4VPhysicsConstructor(name)
{
  ...
  // process: 1 keV by default param->SetLowestElectronEnergy(1.*keV); activate/inactivate
  // integral approach: true by default param->SetIntegral(true); inactivate to use cuts
  // as final range
  param->SetUseCutAsFinalRange(false);

  //
  // MSC options and parameters: 3 different stepping algorithms (can be set from the G4
  // macro)
  // 1. fUseSafety: opt0 step limit [corresponds to G4-Urban fUseSafety]
  param->SetMscStepLimitType(fUseSafety);
  // 2. fUseDistanceToBoundary: opt3 step limit [corresponds to G4-Urban
  // fUseDistanceToBoundary] param->SetMscStepLimitType(fUseDistanceToBoundary);
  // 3. fUseSafetyPlus: error free G4-GS stepping [there is no corresponding G4-Urban]
  // param->SetMscStepLimitType(fUseSafetyPlus);
  // Skin depth: times elastic mean free path skin near boundaries (can be set from the G4
  // macro)
