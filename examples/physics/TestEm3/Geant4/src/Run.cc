...
Run::Run(DetectorConstruction *det)
    : G4Run(), fDetector(det), fParticle(0), fEkin(0.), fChargedStep(0), fNeutralStep(0),
      fN_gamma(0), fN_elec(0), fN_pos(0), fApplyLimit(false)
{
...
void Run::EndOfRun()
{
  ...
  for (G4int k = 1; k <= fDetector->GetNbOfAbsor(); k++) {
    ...
    G4cout << std::setw(14) << fDetector->GetAbsorMaterial(k)->GetName() << ": "
           << std::setprecision(6) << std::setw(6) << G4BestUnit(MeanEAbs, "Energy")
           << " :  " << std::setprecision(6) << std::setw(5)
           << G4BestUnit(rmsEAbs, "Energy") << std::setw(10) << resolution << " +- "
           << std::setw(5) << rmsres << " %" << std::setprecision(6) << std::setw(10)
  ...
  G4cout << std::setprecision(6) << " Mean number of charged steps  " << fChargedStep
         << G4endl;
  ...
  for (G4int j = 1; j <= fDetector->GetNbOfAbsor(); j++) {
    if (fLimittrue[j] < DBL_MAX) {
      ...
      acc.EmAcceptanceGauss("Edep" + mat, nEvt, MeanEAbs, fEdeptrue[j], fRmstrue[j],
                            fLimittrue[j]);
  ...
  for (G4int il = 0; il < nLayers; ++il) {
    G4cout << "      " << std::setw(10) << il << std::setw(20) << std::setprecision(6)
           << fCHTrackLPerLayer[il] * norm / cm << std::setw(20) << std::setprecision(6)
           << fEDepPerLayer[il] * norm / GeV << G4endl;
  }
  G4cout << G4endl;
  G4cout << " \n "
            "============================================================================"
