...
//
//   This is an extended version of the TestEm3 standard Geant4 test used for simplified
//   calorimeter tests. The orginal Geant4 TestEm3 application was taken from:
...
static struct option options[] = {
    {"flag to run the application in performance mode (default FALSE)", no_argument, 0,
     'p'},
...
void help()
{
  std::cout << "\n " << std::setw(100) << std::setfill('=') << "" << std::setfill(' ')
            << std::endl;
  std::cout << "  Extended version of the standard Geant4 TestEM3 simplified calorimeter "
               "test.    \n"
            << "  The corresponding GV application is caloAppRP. \n"
            << std::endl
            << "  **** Parameters: \n"
            << "      -p :   flag  ==> run the application in performance mode i.e. no "
               "scoring \n"
            << "         :   -     ==> run the application in NON performance mode i.e. "
               "with scoring (default) \n"
...
int main(int argc, char **argv)
{
  ...
  //
  //
  // choose the Random engine: set to CLHEP::MTwistEngine => we use vecgeom RNG that is
  // the standard c++11 mtwist
